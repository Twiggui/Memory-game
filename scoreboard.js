...
while (localStorage.getItem(`score${lastItem}`)) {
  //we check all the the existing score0, score1... and we get them in the array
...
let rankingCurrentScore = listObjectsScores.findIndex(
  (i) => i.time === currentScoreObject.time
);
let currentScoreElt = document.querySelector('.currentScore');
currentScoreElt.innerHTML = `Latest score <span>${
  currentScoreObject.score
}</span> for a rank of <span>${rankingCurrentScore + 1}</span> (on a total of ${
  listObjectsScores.length
})`;
